...
: ${AGNOSTER_DIR_FG:=#02A5F0}
: ${AGNOSTER_DIR_BG:=black}
...
prompt_git() {
  ...
  if [[ "$(command git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    local modified_count untracked_count removed_count added_count
    modified_count=$(command git status --porcelain 2>/dev/null | grep '^ M' | wc -l)
    untracked_count=$(command git status --porcelain 2>/dev/null | grep '^??' | wc -l)
    added_count=$(command git status --porcelain 2>/dev/null | grep '^A' | wc -l)
    removed_count=$(command git status --porcelain 2>/dev/null | grep '^ D' | wc -l)
    ...
    local working
    if [[ $modified_count -ne 0 || $added_count -ne 0 || $removed_count -ne 0 || $untracked_count -ne 0 ]]; then
      working=$' \uf044'
    fi

    echo -n "${${ref:gs/%/%%}/refs\/heads\//$PL_BRANCH_CHAR }${mode}${working}"
    [[ $AGNOSTER_GIT_INLINE == 'true' ]] && prompt_git_relative
    [[ $modified_count -ne 0 ]] && prompt_segment "#ff9900" "#000000" "!${modified_count}"
    [[ $added_count -ne 0 ]] && prompt_segment "#ffff00" "#000000" "+${added_count}"
    [[ $removed_count -ne 0 ]] && prompt_segment "#ff0000" "#ffffff" "-${removed_count}"
    [[ $untracked_count -ne 0 ]] && prompt_segment "#01A9F6" "#111111" "?${untracked_count}"
